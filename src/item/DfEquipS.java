...
public class DfEquipS extends DfEqItem {
   
    public static final DfEquipSVal[] vals = DfEquipSVal.values();

    public enum DfEquipSVal {
        Name,
        Cost,
        Def,
        Str,
    ...
    public DfEquipS dupe() {
        DfEquipS item = new DfEquipS();
        copyTo(item);
        return item;
    }

    public void copyTo(DfEquipS item) {
        super.copyTo(item);
    }
    ...
    public static void load(String fileName) {
        try {
            ...
            int r = ExcelUtils.getRowForString("Shield", wb, sheet);
            ...
            for (; r <= numRows; r++) {
                ...
                if (row == null) 
                    continue;
                DfEquipS item = new DfEquipS();
                Data.dfEquipSList.add(item);
                for (int c = 1; c < numCols; c++) {
                    cell = row.getCell(c);
                    if (ExcelUtils.isCellEmpty(cell)) {
                        if (c == 1) { 
                            // No name, so no mon, remove it and move on
                            Data.dfEquipSList.remove(item);
                            break;
                        }
                        continue;
                    }
                    DfEquipS.DfEquipSVal enumValue = DfEquipS.vals[c-1];
                    switch (enumValue) {
                        case Name:
